...
> 2026-07-24 업데이트: 17건에서 21건(CAN 8, AUS 5, GBR 8)으로 재확장했다.
> 2026-07-29 업데이트: 21건에서 25건(CAN 9, AUS 7, GBR 9)으로 재확장했다 — 국가 간 형평성 공백을 메우는 데 초점을
> 맞췄다: 가족동반(`familyAccompanied` 입력값과 직결) 안내 문서가 영국에만 있었으므로 캐나다에 부양 자녀 정의 문서를
> 추가했고, 모든 국가 비자 신청에 공통 적용되는 건강·신원 요건 문서가 호주에는 하나도 없었으므로 2건을 추가했으며,
> 영국의 기존 "급여 할인" 문서를 보완하는 구체적 대상 직업 목록(Immigration Salary List)을 추가했다. 아래 내용은
> 이 확장 이후 기준.
> **2026-07-29 GCP 배포 DB 동기화 완료**: 로컬 native Postgres에서 `policy_documents`/`policy_chunks`를
> `pg_dump --data-only --column-inserts`로 추출해 GCP 배포 DB에 주입했다 (`chat_message_sources`는 `policy_chunks`를
> FK로 참조해 삭제를 막고 있어 먼저 비웠다 — 로컬 기준 0건이라 실제 채팅 기록 손실은 없었음). 배포 DB에서
> `SELECT country_code, count(*) FROM policy_documents GROUP BY country_code`로 확인한 결과 `CAN 9 / AUS 7 / GBR 9`,
> `policy_chunks` 82건으로 로컬과 정확히 일치함을 확인했다.

## 데이터 확인

캐나다·호주·영국 공식 이민 사이트에서 브라우저로 직접 접속해 문서 25건의 본문을 수집했다 (requests 기반 스크래핑은
canada.ca와 immi.homeaffairs.gov.au가 요청을 자주 차단해 불안정했다 — `WebFetch` 툴은 canada.ca에서 403, gov.uk/
homeaffairs에서 DNS 타임아웃을 반환했지만, 실제 브라우저로 접속하면 정상적으로 열렸다).
...
| CAN | 부양 자녀(Dependent Child) 자격 기준 (2026-07-29 신규) | canada.ca |
| AUS | 호주 비자 건강 요건 (Health Requirement) (2026-07-29 신규) | immi.homeaffairs.gov.au |
| AUS | 호주 비자 신원(성격) 요건 (Character Requirement) (2026-07-29 신규) | immi.homeaffairs.gov.au |
| GBR | Immigration Salary List (급여 할인 대상 직업 목록) (2026-07-29 신규) | gov.uk |
...
- 전체 청크 수: **82개** (기존 23개 → 52개 → 70개 → 82개로 확장)
- 청크 길이: 최소 150자, 최대 1029자, 평균 640자 (분포는 `01_chunk_length_distribution.png` 참고)
...
**2026-07-29 확장분(4건) 검증** — 신규 문서를 직접 겨냥한 질문 3건으로 별도 확인했다.

| 국가 | 질문 | 상위 매칭 (유사도) |
|---|---|---|
| CAN | 제 아이가 몇 살까지 부양가족으로 같이 갈 수 있나요? | 부양 자녀 자격 기준 (0.788, 0.760, 0.758 — 상위 3개 전부 신규 문서) |
| AUS | 건강 검진이나 범죄 이력 때문에 비자가 거절될 수 있나요? | 호주 비자 신원(성격) 요건 (0.798, 0.786, 0.765 — 상위 3개 전부 신규 문서) |
| GBR | 제 직업이 급여를 적게 받아도 스폰서를 받을 수 있는 직업 목록에 있나요? | Immigration Salary List (0.787, 신규), Skilled Worker 스폰서 지침 (0.782), 직무·급여 요건 (0.773) |

세 질문 모두 의도한 신규 문서가 1위로 검색됐다 (GBR만 상위 3개 중 1개, 나머지 둘은 상위 3개 전부가 신규 문서 — 해당
주제가 기존 코퍼스에 전혀 없었다는 뜻이기도 하다).
...
- 문서를 국가당 2~3건 → 5~6건 → CAN 8건/AUS 5건/GBR 8건 → CAN 9건/AUS 7건/GBR 9건으로 확장했지만, 여전히 특정
  세부 질문(예: 호주 Subclass 189 스폰서 필요 여부처럼 이 비자 자체가 스폰서 불필요한 독립 비자라는 점을 명확히
  언급하는 문서는 없음)에는 근거 문서가 부족할 수 있다. 호주는 사전 조사했던 원래 공식 링크 목록(비자 규칙 구현용)의
  후보는 이미 모두 소진된 상태였고, 2026-07-29 확장분은 그 목록 밖에서 새로 조사한 문서(건강/신원 요건)다 — 추가
  확장을 하려면 계속 새 공식 문서를 조사하는 단계부터 필요하다.
- 2026-07-29 확장은 임의로 문서 수만 늘리기보다, 국가 간 커버리지 형평성 공백(가족동반은 GBR에만, 건강/신원 요건은
  어느 국가에도 없었음)을 우선 기준으로 삼았다. 캐나다도 배우자/파트너 동반 관련 공식 문서(Express Entry 경제이민
  기준 부양가족과는 별개로, 가족 초청 이민(family sponsorship) 카테고리 문서만 검색됨 — 개념이 달라 이번엔 제외)는
  아직 없어 다음 확장 후보로 남아있다.
