...
from app.career_matching import get_career_similarity
...
def recommend(request: AiRecommendRequest) -> AiRecommendResponse:
    career_text = request.user_profile.get("careerText") or ""
    ...
    for country in request.supported_countries:
        ...
        career = get_career_similarity(country, career_text)
        ...
        total_score = (
            ...
            + career["score"] * SCORE_WEIGHTS["career"]
            + pref_score * SCORE_WEIGHTS["preference"]
        )

        strengths = list(rule_result["strengths"])
        if career["topMatch"]:
            strengths.append(f"경력과 유사한 직업군: {career['topMatch']['title']}")
        ...
        results.append(
            CountryResult(
                ...
                career_similarity=career["score"],
    ...
    return AiRecommendResponse(
        ...
        model_version="rule1.0+kmeans1.0+e5career1.0",
